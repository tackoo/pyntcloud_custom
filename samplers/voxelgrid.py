...
class VoxelgridCentersSampler(VoxelgridSampler):
    ...
    def compute(self):
        df = pd.DataFrame(self.voxelgrid.voxel_centers[np.unique(self.voxelgrid.voxel_n)], columns=["x", "y", "z"])
        df2 = pd.DataFrame(self.pyntcloud.points, columns=["red", "green", "blue", "scalarf"])
        df2 ["voxel_n"] = self.voxelgrid.voxel_n
        grouped_df2 = df2.groupby("voxel_n").mean().astype(int)
        indexed_df2 = grouped_df2.reset_index(drop=True)    
        df["red"] = indexed_df2["red"]
        df["green"] = indexed_df2["green"]
        df["blue"] = indexed_df2["blue"]
        df["scalarf"] = indexed_df2["scalarf"]
        return df
...
class VoxelgridCentroidsSampler(VoxelgridSampler):
    ...
    def compute(self):
        ...
        df2 = pd.DataFrame(self.pyntcloud.points, columns=["red", "green", "blue"])
